...
export const APOLLO_CITY_PREFIX = "apollo-sales-execution-services-in-";

export function toApolloCitySlug(slug) {
  if (!slug) return "";

  const cleanSlug = slug.trim().replace(/^\/|\/$/g, "");

  return cleanSlug.startsWith(APOLLO_CITY_PREFIX)
    ? cleanSlug
    : `${APOLLO_CITY_PREFIX}${cleanSlug}`;
}

export async function getApolloCityMetadata(slug) {
  const pageSlug = toApolloCitySlug(slug);
  const page = await getApolloCityPage(pageSlug);
  ...
  return {
    ...
    openGraph: {
      title: seo?.ogTitle || seo?.metaTitle || page.title || process.env.SITE_TITLE,
      description:
      ...
      images: ogImageUrl
        ? [
            {
              url: ogImageUrl,
              alt: seo?.ogTitle || seo?.metaTitle || page.title,
            },
          ]
        : [],
    },
...
export default async function ApolloCityPage({ slug }) {
  const page = await getApolloCityPage(toApolloCitySlug(slug));
    ...
    ? {
        ...
        image: page.hero_Section.image
          ? {
              ...
              alt: page.hero_Section.image.alternativeText || "",
            }
          : null,
      }
    : null;
    ...
    ? {
        ...
        image: page.why_city_section.image
          ? {
              ...
              src: CMS + page.why_city_section.image.url,
              alt: page.why_city_section.image.alternativeText || "",
            }
          : null,
      }
    : null;
  ...
  return (
    <>
      ...
      <Section3 tools={page.tool_section} city={page.cityName} />

      <Section4 data={processData} />

      {whyCityData && <Section5 data={whyCityData} />}

      {page.cta_section && <Section6 {...page.cta_section} />}

      <Section7Accordion items={page.faq_item || []} city={page.cityName} />
    </>
  );
}
