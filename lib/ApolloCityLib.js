"use server";

const CMS =
  process.env.NEXT_PUBLIC_CMS_URL || process.env.CMS_URL;
const APOLLO_CITY_PREFIX = "apollo-sales-execution-services-in-";

export async function getApolloCityPage(slug) {
  const cleanSlug = slug.trim();
  const fallbackSlug = cleanSlug.startsWith(APOLLO_CITY_PREFIX)
    ? cleanSlug.slice(APOLLO_CITY_PREFIX.length)
    : null;

  const page = await fetchApolloCityPage(cleanSlug);

  if (page || !fallbackSlug) {
    return page;
  }

  return fetchApolloCityPage(fallbackSlug);
}

async function fetchApolloCityPage(slug) {
  const url =
    `${CMS}/api/apollo-city-pages` +
    `?filters[slug][$eq]=${encodeURIComponent(slug)}` +
    `&populate[hero_Section][populate]=*` +
    `&populate[service_Item][populate]=*` +
    `&populate[tool_section][populate][tool_item][populate]=*` +
    `&populate[process_item][populate]=*` +
    `&populate[why_city_section][populate]=*` +
    `&populate[cta_section][populate]=*` +
    `&populate[faq_item][populate]=*` +
    `&populate[seo][populate]=*`;

  const res = await fetch(url, {
    next: { tags: ["apollo-city-pages"], revalidate: 60 },
  });

  if (!res.ok) {
    console.error("Strapi error:", res.status);
    return null;
  }

  const json = await res.json();

  if (!json?.data || json.data.length === 0) {
    return null;
  }

  return json.data[0];
}
